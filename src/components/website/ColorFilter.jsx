...
const ColorFilter =  () => {
    ...
    const[selcolor,setSlecolor]=useState("");
            ...
            <div>
                <ul className="space-y-1 text-sm flex gap-4 flex-wrap text-gray-700">
                    {colors.map((item, index) => (
                        <li onClick={() => setSlecolor(item.slug)} style={{background: item.hexcode}} key={index} className="cursor-pointer w-6 h-6 rounded-xl space-y-3 hover:font-bold flex justify-between ">
